...
local hotdogLoopRunning = false

local function HotdogLoop()
    if hotdogLoopRunning then return end -- bylo: nowe watki przy kazdym podniesieniu stoiska
    hotdogLoopRunning = true
    ...
    CreateThread(function()
        while StandObject ~= nil do
            Wait(IsPushing and 0 or 250)
            if IsPushing then
                DisableControlAction(0, 244) -- m
                DisableControlAction(0, 23) -- f
            end
        end
        hotdogLoopRunning = false
    end)
