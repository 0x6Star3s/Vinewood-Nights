-- Variables
---Czy `src` jest policjantem na sluzbie
local function IsCop(Player)
    return Player and Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty
end

---Czy cel jest skuty / martwy (bez tego przeszukanie i konfiskata to darmowy grabiez-button)
local function IsRestrained(Player)
    if not Player then return false end
    local meta = Player.PlayerData.metadata
    return meta["ishandcuffed"] or meta["isdead"] or meta["inlaststand"]
end
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or not IsCop(Player) then return end

    price = tonumber(price)
    if not price or price <= 0 or price % 1 ~= 0 or price > 100000 then return end -- mandat dowolnej wysokosci
    if OtherPlayer.PlayerData.money["bank"] < price then
...
RegisterNetEvent('police:server:SearchPlayer', function(playerId)
    ...
    if not IsCop(QBCore.Functions.GetPlayer(src)) or not IsRestrained(SearchedPlayer) then return end
...
RegisterNetEvent('police:server:SeizeCash', function(playerId)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local SearchedPlayer = QBCore.Functions.GetPlayer(playerId)
    if not IsCop(Player) or not IsRestrained(SearchedPlayer) then return end
...
RegisterNetEvent('police:server:RobPlayer', function(playerId)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local SearchedPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not IsRestrained(SearchedPlayer) then return end
...
RegisterNetEvent('police:server:FillVehicleTrunk', function(plate)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or Player.PlayerData.job.name ~= 'police' or not Player.PlayerData.job.onduty then return end
    if type(plate) ~= 'string' then return end
    exports['qb-inventory']:SetTrunkItems(plate, Config.CarItems)
end)
