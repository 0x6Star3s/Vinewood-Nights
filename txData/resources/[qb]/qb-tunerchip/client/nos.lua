...
CreateThread(function()
    while true do
        ...
        Wait(100) -- bylo 3 ms: odczyt tablicy rejestracyjnej 300x/s
    end
end)
...
CreateThread(function()
    while true do
        if NitrousActivated then
            local veh = GetVehiclePedIsIn(PlayerPedId())
            if veh ~= 0 then
                if not flamesSynced then -- bylo: event sieciowy co klatke
                    flamesSynced = true
        ...
        Wait(NitrousActivated and 0 or 250) -- w spoczynku nie ma czego liczyc co klatke
    end
end)
