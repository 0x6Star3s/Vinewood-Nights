...
CreateThread(function()
    -- ponytail: jednorazowe ladowanie, bylo `while true ... Wait(7)` krecace sie w kolko
...
---Posrednik nieruchomosci? (tworzenie/kasowanie domow to nie zabawa dla kazdego)
local function IsRealtor(src)
    local Player = QBCore.Functions.GetPlayer(src)
    return Player and Player.PlayerData.job.name == "realestate"
end

RegisterNetEvent('qb-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    if not IsRealtor(src) or type(street) ~= 'string' then return end
    street = street:gsub("%'", "")
    price = tonumber(price)
    tier = tonumber(tier)
    if not price or price <= 0 or not tier then return end
...
RegisterNetEvent('qb-houses:server:addGarage', function(house, coords)
    local src = source
    if not IsRealtor(src) or not Config.Houses[house] then return end
...
RegisterNetEvent('qb-houses:server:buyHouse', function(house)
    ...
    if (bankBalance >= HousePrice) then
        ...
        exports['qb-management']:AddMoney("realestate", math.floor((HousePrice / 100) * math.random(18, 25)))
...
RegisterNetEvent('qb-houses:server:lockHouse', function(bool, house)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player or not hasKey(Player.PlayerData.license, Player.PlayerData.citizenid, house) then return end
...
RegisterNetEvent('qb-houses:server:giveKey', function(house, target)
    ...
    -- klucze rozdaje tylko wlasciciel
    if not Owner or not pData or not housekeyholders[house] then return end
    if houseownercid[house] ~= Owner.PlayerData.citizenid then return end
...
RegisterNetEvent('qb-houses:server:removeHouseKey', function(house, citizenData)
    ...
    if not Owner or type(citizenData) ~= 'table' then return end
    if houseownercid[house] ~= Owner.PlayerData.citizenid then return end
...
RegisterNetEvent('qb-houses:server:deletehouses', function(selectedHouse)
    if not IsRealtor(source) or type(selectedHouse) ~= 'table' then return end
