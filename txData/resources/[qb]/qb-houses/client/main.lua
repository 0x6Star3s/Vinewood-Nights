...
local function DrawActionText(coords, text)
...
local function DrawActionMarker(coords, text, distance)
    if not Config.ActionMarkers or not Config.ActionMarkers.enabled then return end

    local marker = Config.ActionMarkers
    DrawMarker(
        marker.markerType,
    ...
    if distance <= marker.textDistance then
        DrawActionText(vector3(coords.x, coords.y, coords.z + (marker.textZOffset or 0.75)), text)
    end
end
...
CreateThread(function()
    while true do
        ...
        if Config.ActionMarkers and Config.ActionMarkers.enabled and LocalPlayer.state.isLoggedIn then
            local ped = PlayerPedId()
            local pos = GetEntityCoords(ped)

            if not IsInside and ClosestHouse and Config.Houses[ClosestHouse] and Config.Houses[ClosestHouse].coords then
                local enter = Config.Houses[ClosestHouse].coords.enter
                local coords = vector3(enter.x, enter.y, enter.z)
                local distance = #(pos - coords)

                if distance <= Config.ActionMarkers.drawDistance then
                    wait = 0
                    DrawActionMarker(coords, "Dom - podejdz blizej aby otworzyc menu", distance)
                end
            elseif IsInside then
                local markerData = {}

                if CurrentHouse and Config.Houses[CurrentHouse] and Config.Houses[CurrentHouse].coords and POIOffsets then
                    local enter = Config.Houses[CurrentHouse].coords.enter
                    markerData[#markerData + 1] = {
                        coords = vector3(enter.x + POIOffsets.exit.x, enter.y + POIOffsets.exit.y, enter.z - Config.MinZOffset + POIOffsets.exit.z + 1.0),
                        text = "Wyjscie z domu"
                    }
                end

                if stashLocation then
                    markerData[#markerData + 1] = { coords = vector3(stashLocation.x, stashLocation.y, stashLocation.z), text = "Schowek [E]" }
                end
                if outfitLocation then
                    markerData[#markerData + 1] = { coords = vector3(outfitLocation.x, outfitLocation.y, outfitLocation.z), text = "Ubrania [E]" }
                end
                if logoutLocation then
                    markerData[#markerData + 1] = { coords = vector3(logoutLocation.x, logoutLocation.y, logoutLocation.z), text = "Zmiana postaci [E]" }
                end

                for _, marker in pairs(markerData) do
                    local distance = #(pos - marker.coords)
                    if distance <= Config.ActionMarkers.drawDistance then
                        wait = 0
                        DrawActionMarker(marker.coords, marker.text, distance)
                    end
                end
            end
        end
