...
RegisterNetEvent('qb-drugs:server:updateDealerItems', function(itemData, amount, dealer)
    ...
    if not Player or type(itemData) ~= 'table' then return end

    amount = tonumber(amount)
    local product = Config.Dealers[dealer] and Config.Dealers[dealer].products[itemData.slot]
    if not product or not amount or amount <= 0 or amount % 1 ~= 0 then return end
...
-- [src] = { index = numer pozycji z Config.DeliveryItems, amount = ile bylo wydane }
...
RegisterNetEvent('qb-drugs:server:giveDeliveryItems', function(deliveryData)
    ...
    if not Player or type(deliveryData) ~= 'table' then return end
    if activeDeliveries[src] then return end -- jedna dostawa naraz

    local index = tonumber(deliveryData.item)
    local cfg = index and Config.DeliveryItems[index]
    if not cfg then return end

    local amount = math.floor(tonumber(deliveryData.amount) or 0)
    if amount < 1 or amount > 3 then return end -- klient losuje 1-3, wiecej nie ma prawa

    activeDeliveries[src] = { index = index, amount = amount }

    Player.Functions.AddItem(cfg.item, amount)
...
RegisterNetEvent('qb-drugs:server:successDelivery', function(deliveryData, inTime)
    ...
    -- pozycja, ilosc i wyplata z serwera (klient dostarcza tylko dane do maila)
    local delivery = activeDeliveries[src]
    if not delivery then return end
    activeDeliveries[src] = nil

    local cfg = Config.DeliveryItems[delivery.index]
    if not cfg then return end

    local item = cfg.item
    local itemAmount = delivery.amount
    local payout = QBCore.Functions.ScalePayout(cfg.payout * itemAmount) -- mnoznik ekonomii (/boost)
    ...
    if inTime then
        if invItem and invItem.amount >= itemAmount then -- on time correct amount
            ...
            if invItem then
                ...
                local modifiedPayout = cfg.payout * newItemAmount
...
---Asortyment dealera z Configu (qb-inventory bierze stad cene zamiast od klienta)
exports('GetDealerProducts', function(dealerName)
    for _, dealer in pairs(Config.Dealers) do
        if dealer.name == dealerName then return dealer.products end
    end
end)
