...
RegisterServerEvent('qb-traphouse:server:RobNpc', function(Traphouse)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Config.TrapHouses[Traphouse] then return end

    local now = os.time()
    if lastNpcRob[src] and now - lastNpcRob[src] < 30 then return end
    lastNpcRob[src] = now
    ...
    if Chance == odd then
        ...
        local amount = QBCore.Functions.ScalePayout(math.random(1, 80)) -- mnoznik ekonomii (/boost)
        Player.Functions.AddMoney('cash', amount, 'traphouse-npc-rob')
    end
end)
