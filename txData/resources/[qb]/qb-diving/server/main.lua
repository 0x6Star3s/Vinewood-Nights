...
RegisterNetEvent('qb-diving:server:SellCoral', function()
    ...
    if hasCoral(src) then
        for _, v in pairs(availableCoral) do
            ...
            Player.Functions.AddMoney('cash', QBCore.Functions.ScalePayout(reward), "sold-coral") -- mnoznik ekonomii (/boost)
...
RegisterNetEvent('qb-diving:server:TakeCoral', function(area, coral, bool)
    ...
    local now = os.time()
    if lastCoral[src] and now - lastCoral[src] < 5 then return end
    lastCoral[src] = now
