...
RegisterNetEvent('qb-occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or type(vehicleData) ~= 'table' then return end

    vehiclePrice = tonumber(vehiclePrice)
    if not vehiclePrice or vehiclePrice <= 0 or vehiclePrice % 1 ~= 0 then return end

    -- wystawic mozna tylko wlasne auto
    local owned = MySQL.query.await('SELECT plate, vehicle FROM player_vehicles WHERE plate = ? AND citizenid = ?', { vehicleData.plate, Player.PlayerData.citizenid })
    if not owned or not owned[1] then return end
    vehicleData.model = owned[1].vehicle
...
RegisterNetEvent('qb-occasions:server:sellVehicleBack', function(vehData)
    ...
    -- model i wlasciciel z bazy, nie z klienta
    local owned = MySQL.query.await('SELECT vehicle FROM player_vehicles WHERE plate = ? AND citizenid = ?', { plate, Player.PlayerData.citizenid })
    if not owned or not owned[1] then return end

    local price = 0
    local model = owned[1].vehicle
    if QBCore.Shared.Vehicles[model] then
        price = tonumber(QBCore.Shared.Vehicles[model].price) or 0
    end
    if price <= 0 then return end
    local payout = math.floor(price * 0.5) -- This will give you half of the cars value
    MySQL.query('DELETE FROM player_vehicles WHERE plate = ? AND citizenid = ?', { plate, Player.PlayerData.citizenid })
    Player.Functions.AddMoney('bank', payout, 'occasion-sell-back')
