...
---Dane kredytu zawsze z bazy i tylko dla wlasnego auta (bylo: saldo i raty z klienta)
local function GetOwnedFinance(citizenid, plate)
    if type(plate) ~= 'string' then return nil end
    local result = MySQL.query.await('SELECT balance, paymentsleft, paymentamount FROM player_vehicles WHERE plate = ? AND citizenid = ?', { plate, citizenid })
...
RegisterNetEvent('qb-vehicleshop:server:financePayment', function(paymentAmount, vehData)
    ...
    local finance = GetOwnedFinance(player.PlayerData.citizenid, plate)
    if not finance then return end

    vehData = {
        balance = tonumber(finance.balance) or 0,
        paymentsLeft = tonumber(finance.paymentsleft) or 0,
        paymentAmount = tonumber(finance.paymentamount) or 0,
        vehiclePlate = plate,
    }

    paymentAmount = tonumber(paymentAmount)
    if not paymentAmount or paymentAmount <= 0 or paymentAmount > vehData.balance then return end

    local minPayment = vehData.paymentAmount
...
RegisterNetEvent('qb-vehicleshop:server:financePaymentFull', function(data)
    ...
    local finance = GetOwnedFinance(player.PlayerData.citizenid, vehPlate)
    if not finance then return end

    local vehBalance = tonumber(finance.balance) or 0 -- saldo z bazy, nie z klienta
...
---Czy ten model wolno w ogole kupic w salonie (audyt ekonomii pkt 13: szly tez wojsko/samoloty)
local function IsBuyable(vehicle)
    local data = vehicle and QBCore.Shared.Vehicles[vehicle]
    return data ~= nil and data.shop ~= nil and data.shop ~= 'none' and tonumber(data.price) ~= nil
end

RegisterNetEvent('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
    vehicle = type(vehicle) == 'table' and vehicle.buyVehicle or vehicle
    local pData = QBCore.Functions.GetPlayer(src)
    if not pData or not IsBuyable(vehicle) then return end
...
RegisterNetEvent('qb-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    ...
    local pData = QBCore.Functions.GetPlayer(src)
    if not pData or not IsBuyable(vehicle) then return end
    if not downPayment or not paymentAmount or paymentAmount < 1 or paymentAmount % 1 ~= 0 then return end -- bylo: 0 rat = dzielenie przez zero
    if downPayment < 0 then return end
    local cid = pData.PlayerData.citizenid
    local cash = pData.PlayerData.money['cash']
    local bank = pData.PlayerData.money['bank']
    local vehiclePrice = QBCore.Shared.Vehicles[vehicle]['price']
...
RegisterNetEvent('qb-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        local vehicle = data
        if not IsBuyable(vehicle) then return end
        local vehiclePrice = QBCore.Shared.Vehicles[vehicle]['price']
...
RegisterNetEvent('qb-vehicleshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        paymentAmount = tonumber(paymentAmount)
        if not IsBuyable(vehicle) then return end
        if not downPayment or not paymentAmount or paymentAmount < 1 or paymentAmount % 1 ~= 0 or downPayment < 0 then return end
        local cid = target.PlayerData.citizenid
        local cash = target.PlayerData.money['cash']
        local bank = target.PlayerData.money['bank']
        local vehiclePrice = QBCore.Shared.Vehicles[vehicle]['price']
