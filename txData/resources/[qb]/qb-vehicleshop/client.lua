...
-- Ile razy zignorować powrót do menu głównego wysyłany przez serwer po podmianie
-- auta w salonie (podgląd strzałkami ma zostawić otwartą listę pojazdów).
local pendingHomeSuppress = 0
...
-- Model wystawiony na najbliższym stanowisku (albo nil, jeśli gracz jest poza
-- salonem / stanowisko nie istnieje).
local function getShowroomModel()
    local shop = insideShop and Config.Shops[insideShop]
    local slot = shop and shop["ShowroomVehicles"] and shop["ShowroomVehicles"][ClosestVehicle]
    return slot and slot.chosenVehicle or nil
end

-- Dane z QBCore.Shared.Vehicles bywają niekompletne (model spoza listy) - brak
-- tego zabezpieczenia wywalał pętlę budującą menu salonu i menu znikało całkiem.
local function getShowroomData()
    local model = getShowroomModel()
    return model and QBCore.Shared.Vehicles[model] or nil
end

local function getVehName()
    local data = getShowroomData()
    return data and data.name or getShowroomModel() or "???"
end

local function getVehPrice()
    local data = getShowroomData()
    return comma_value(data and data.price or 0)
end

local function getVehBrand()
    local data = getShowroomData()
    return data and data.brand or ""
end
...
RegisterNetEvent('qb-vehicleshop:client:homeMenu', function()
    if pendingHomeSuppress > 0 then
        pendingHomeSuppress = pendingHomeSuppress - 1
        return
    end
...
-- Podgląd auta w salonie strzałkami: podmienia wystawiony pojazd i zostawia
-- otwartą listę (serwerowe menu główne po podmianie jest ignorowane).
RegisterNetEvent('qb-vehicleshop:client:previewVeh', function(data)
    pendingHomeSuppress = pendingHomeSuppress + 1
    ...
    -- selected przekazane jawnie: podmiana trwa jeszcze chwilę, a zaznaczenie ma
    -- zostać na aucie wybranym strzałką, nie skakać na poprzednie.
    TriggerEvent('qb-vehicleshop:client:openVehCats', { catName = data.catName, selected = data.toVehicle })
end)

RegisterNetEvent('qb-vehicleshop:client:openVehCats', function(data)
    local shown = data.selected or getShowroomModel()
    local vehMenu = {
        {
            header = Lang:t('menus.goback_header'),
            icon = "fa-solid fa-angle-left",
                ...
                for _, shop in pairs(QBCore.Shared.Vehicles[k]["shop"]) do
                    if shop == insideShop then
                        vehMenu[#vehMenu + 1] = {
                            ...
                            params = {
                                event = 'qb-vehicleshop:client:previewVeh',
                                args = {
                                    toVehicle = v.model,
...
-- Podmiana auta wystawowego. Kolejkowana (szybkie przełączanie strzałkami
-- kasowało auto w trakcie spawnu i salon zostawał pusty) i z walidacją modelu
-- (nieistniejący model zawieszał pętlę HasModelLoaded na zawsze).
...
RegisterNetEvent('qb-vehicleshop:client:swapVehicle', function(data)
    ...
    local slot = Config.Shops[shopName]["ShowroomVehicles"][data.ClosestVehicle]
    if not slot or slot.chosenVehicle == data.toVehicle then return end

    local model = GetHashKey(data.toVehicle)
    if not IsModelInCdimage(model) or not IsModelAVehicle(model) then
        print(("^1[qb-vehicleshop]^7 Model '%s' nie istnieje - pomijam podmianę."):format(tostring(data.toVehicle)))
        return
    end
    ...
    CreateThread(function()
        ...
        local closestVehicle, closestDistance = QBCore.Functions.GetClosestVehicle(vector3(coords.x, coords.y, coords.z))
        if closestVehicle ~= 0 and closestDistance < 5 then
            DeleteEntity(closestVehicle)
            local timeout = GetGameTimer() + 3000
            while DoesEntityExist(closestVehicle) and GetGameTimer() < timeout do Wait(50) end
        end
        ...
        if HasModelLoaded(model) then
            ...
            local spawnTimeout = GetGameTimer() + 3000
            while not DoesEntityExist(veh) and GetGameTimer() < spawnTimeout do Wait(50) end
            SetModelAsNoLongerNeeded(model)
            ...
            print(("^1[qb-vehicleshop]^7 Nie udało się wczytać modelu '%s'."):format(tostring(data.toVehicle)))
        end

        swapInProgress = false
        local nextSwap = pendingSwap
        pendingSwap = nil
        if nextSwap then TriggerEvent('qb-vehicleshop:client:swapVehicle', nextSwap) end
    end)
end)
