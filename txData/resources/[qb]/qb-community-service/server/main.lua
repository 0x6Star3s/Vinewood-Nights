...
AddEventHandler('qb-communityservice:finishCommunityService', function(key)
	local src = source 
		...
		-- konczy sie samo dopiero, gdy licznik zszedl do zera
		if xPlayer and (xPlayer.PlayerData.metadata["communityservice"] or 0) <= 0 then
...
AddEventHandler('qb-communityservice:server:StartCommunityService', function(targetSourceId, sentence, key)
	...
	local count = tonumber(sentence)

		local Officer = QBCore.Functions.GetPlayer(src)
		local tPlayer = QBCore.Functions.GetPlayer(player)
		if not Officer or not tPlayer then return end
		if Officer.PlayerData.job.name ~= 'police' or not Officer.PlayerData.job.onduty then return end
		if not count or count < 1 or count % 1 ~= 0 or count > 100 then return end
