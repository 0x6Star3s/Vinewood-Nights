...
function DeleteGivenVehicle( veh, timeoutMax )
    ...
    -- pod OneSync auto moze nalezec do innego klienta - bez kontroli DeleteVehicle nic nie zrobi
    if ( not NetworkHasControlOfEntity( veh ) ) then
        NetworkRequestControlOfEntity( veh )

        while ( not NetworkHasControlOfEntity( veh ) and timeout < timeoutMax ) do
            timeout = timeout + 1
            Citizen.Wait( 100 )
        end
...
function GetVehicleInDirection( entFrom, coordFrom, coordTo )
	local rayHandle = StartShapeTestCapsule( coordFrom.x, coordFrom.y, coordFrom.z, coordTo.x, coordTo.y, coordTo.z, 1.5, 10, entFrom, 7 )
