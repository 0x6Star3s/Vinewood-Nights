...
local menu16 = MenuV:CreateMenu(false, Lang:t("menu.set_my_job"), menuLocation, 220, 20, 60, 'size-125', 'none', 'menuv', 'test16')
local menu17 = MenuV:CreateMenu(false, Lang:t("menu.job_grades"), menuLocation, 220, 20, 60, 'size-125', 'none', 'menuv', 'test17')
...
local menu2_admin_set_job = menu2:AddButton({
    icon = '💼',
    label = Lang:t("menu.set_my_job"),
    value = menu16,
    description = Lang:t("desc.set_my_job_desc")
})
...
local menu5_vehicles_keys = menu5:AddButton({
    icon = '🔑',
    label = Lang:t("menu.car_keys"),
    value = 'carkeys',
    description = Lang:t("desc.car_keys_desc")
})
...
-- Ustaw sobie prace: lista z QBCore.Shared.Jobs, potem rangi tej pracy, potem /setjob na wlasnym id
local function OpenJobGradesMenu(jobName, job)
    menu17:ClearItems()
    MenuV:OpenMenu(menu17)
    local grades = {}
    for g in pairs(job.grades) do grades[#grades + 1] = tonumber(g) end
    table.sort(grades)
    for _, g in ipairs(grades) do
        local grade = job.grades[tostring(g)]
        menu17:AddButton({
            label = ('%d - %s'):format(g, grade.name),
            value = g,
            description = grade.payment and ('$' .. grade.payment .. (grade.isboss and ' - szef' or '')) or '',
            select = function(_)
                TriggerServerEvent('QBCore:CallCommand', 'setjob', { GetPlayerServerId(PlayerId()), jobName, g })
                QBCore.Functions.Notify(('%s - %s'):format(job.label, grade.name), 'success')
            end
        })
    end
end

menu2_admin_set_job:On('Select', function(_)
    menu16:ClearItems()
    local names = {}
    for name in pairs(QBCore.Shared.Jobs) do names[#names + 1] = name end
    table.sort(names)
    for _, name in ipairs(names) do
        local job = QBCore.Shared.Jobs[name]
        local n = 0
        for _ in pairs(job.grades) do n = n + 1 end
        menu16:AddButton({
            label = job.label,
            value = name,
            description = ('%s - rang: %d'):format(name, n),
            select = function(_) OpenJobGradesMenu(name, job) end
        })
    end
end)
...
menu5_vehicles_keys:On('Select', function(_)
    TriggerServerEvent('QBCore:CallCommand', "carkeys", {})
end)
