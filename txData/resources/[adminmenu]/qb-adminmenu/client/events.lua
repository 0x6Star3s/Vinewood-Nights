...
RegisterNetEvent('qb-admin:client:vehDiag', function()
    local veh = GetVehiclePedIsIn(PlayerPedId(), false)
    if veh == 0 then
        QBCore.Functions.Notify(Lang:t("error.no_vehicle"), 'error')
        return
    end
    local H = function(f) return GetVehicleHandlingFloat(veh, 'CHandlingData', f) end
    local model = GetEntityModel(veh)
    local flatVel = H('fInitialDriveMaxFlatVel')
    local lines = {
        ('model %s  tablica %s'):format(GetDisplayNameFromVehicleModel(model), GetVehicleNumberPlateText(veh)),
        ('biegi: skrzynia=%d  aktualny=%d  najwyzszy=%d'):format(
            GetVehicleHandlingInt(veh, 'CHandlingData', 'nInitialDriveGears'),
            GetVehicleCurrentGear(veh), GetVehicleHighGear(veh)),
        ('flatVel=%.2f  (teoretycznie %.0f km/h)  szacowana maks=%.0f km/h'):format(
            flatVel, flatVel * 1.32, GetVehicleEstimatedMaxSpeed(veh) * 3.6),
        ('driveForce=%.4f  driveInertia=%.3f  biasFront=%.2f'):format(
            H('fInitialDriveForce'), H('fDriveInertia'), H('fDriveBiasFront')),
        ('brakeForce=%.4f  steeringLock=%.2f  drag=%.2f'):format(
            H('fBrakeForce'), H('fSteeringLock'), H('fInitialDragCoeff')),
        ('silnik=%.0f  nadwozie=%.0f  benzyna=%.0f  jezdny=%s'):format(
            GetVehicleEngineHealth(veh), GetVehicleBodyHealth(veh), GetVehiclePetrolTankHealth(veh),
            tostring(IsVehicleDriveable(veh, false))),
        ('silnik_dziala=%s  klucze=%s  reczny=%s  gaz(71)=%s  hamulec/wsteczny(72)=%s'):format(
            tostring(GetIsVehicleEngineRunning(veh)),
            tostring(exports['qb-vehiclekeys']:HasKeys(QBCore.Functions.GetPlate(veh))),
            tostring(GetVehicleHandbrake(veh)),
            IsControlEnabled(2, 71) and 'wlaczony' or 'ZABLOKOWANY',
            IsControlEnabled(2, 72) and 'wlaczony' or 'ZABLOKOWANY'),
    }
    print('--- vehdiag ---')
    for _, l in ipairs(lines) do
        print(l)
        TriggerEvent('chat:addMessage', { args = { 'vehdiag', l } })
    end
end)

RegisterNetEvent('qb-admin:client:giveCarKeys', function()
