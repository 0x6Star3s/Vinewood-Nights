...
-- Lista graczy budowana na zadanie (bylo: watek co 1.5 s, ktory umieral na pierwszym nil-u)
local function GetPlayerList()
    local list = {}
    for _, v in pairs(QBCore.Functions.GetPlayers()) do
        local ped = QBCore.Functions.GetPlayer(v)
        if ped and ped.PlayerData.charinfo then
            local fullname = (ped.PlayerData.charinfo.firstname or '') .. ' ' .. (ped.PlayerData.charinfo.lastname or '')
            list[#list + 1] = {
                name = fullname .. ' | (' .. (GetPlayerName(v) or '') .. ')',
                id = v,
...
RegisterNetEvent('qb-admin:server:GetPlayersForBlips', function()
    local src = source
    if not QBCore.Functions.HasPermission(src, 'admin') and not IsPlayerAceAllowed(src, 'command') then return end
    TriggerClientEvent('qb-admin:client:Show', src, GetPlayerList())
end)
...
QBCore.Commands.Add('vehdiag', 'Wypisz zywe wartosci handlingu auta, w ktorym siedzisz (Admin)', {}, false, function(source)
    TriggerClientEvent('qb-admin:client:vehDiag', source)
end, 'admin')

QBCore.Commands.Add('carkeys', Lang:t("desc.car_keys_desc"), {}, false, function(source)
    local src = source
    TriggerClientEvent('qb-admin:client:giveCarKeys', src)
end, 'admin')
