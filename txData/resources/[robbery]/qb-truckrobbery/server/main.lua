...
local lastVanRobbery = {}

RegisterServerEvent('AttackTransport:graczZrobilnapad', function()
	local _source = source
	local xPlayer = QBCore.Functions.GetPlayer(_source)
	if not xPlayer then return end
	if ActiveMission ~= 1 then return end -- napad musi faktycznie trwac

	local now = os.time()
	if lastVanRobbery[_source] and now - lastVanRobbery[_source] < 60 then return end
	lastVanRobbery[_source] = now
