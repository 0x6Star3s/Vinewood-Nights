...
RegisterNetEvent('qb-bankrobbery:server:setBankState', function(bankId)
    ...
    -- Minimum policji sprawdzane po stronie serwera (u klienta to tylko podpowiedz)
    local minPolice = Config.GetMinimumPolice(bankId)
    if QBCore.Functions.GetDutyCount('police') < minPolice then
        TriggerClientEvent('QBCore:Notify', source, ('Za malo policji na sluzbie (potrzeba %d)'):format(minPolice), 'error')
        return
    end
    if bankId == "paleto" then
        ...
        TriggerEvent('qb-bankrobbery:server:setTimeout', 'paleto')
    elseif bankId == "pacific" then
        ...
        TriggerEvent('qb-bankrobbery:server:setTimeout', 'pacific')
    else
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 ...
                 elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = Config.GetBillWorth('fleeca')
                    }
    ...
    elseif type == "paleto" then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 ...
                 elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = Config.GetBillWorth('paleto')
                    }
    ...
    elseif type == "pacific" then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = Config.GetBillWorth('pacific')
                    }
                    ply.Functions.AddItem('markedbills', math.random(1,4), false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], "add")
                end
...
AddEventHandler('qb-bankrobbery:server:setTimeout', function(bankId)
    if robberyBusy or timeOut then return end
    timeOut = true
    bankId = (bankId == 'paleto' or bankId == 'pacific') and bankId or 'pacific'
    CreateThread(function()
        -- cooldown z QBConfig.Economy.Heists (qb-core/config.lua)
        SetTimeout(Config.GetHeistCooldown(bankId), function()
            for k in pairs(Config.BigBanks[bankId]["lockers"]) do
                Config.BigBanks[bankId]["lockers"][k]["isBusy"] = false
                Config.BigBanks[bankId]["lockers"][k]["isOpened"] = false
            end
            TriggerClientEvent('qb-bankrobbery:client:ClearTimeoutDoors', -1)
            Config.BigBanks[bankId]["isOpened"] = false
            timeOut = false
            robberyBusy = false
...
AddEventHandler('qb-bankrobbery:server:SetSmallBankTimeout', function(bankId)
    ...
    timeOut = true
    CreateThread(function()
        SetTimeout(Config.GetHeistCooldown(bankId), function() -- cooldown z QBConfig.Economy.Heists
