if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    ---Saldo i wlasciciel stacji zawsze z bazy - klient podaje tylko kwote i lokalizacje
    local function GetStation(location)
        local result = MySQL.Sync.fetchAll('SELECT owner, balance FROM fuel_stations WHERE `location` = ?', { location })
    ...
    RegisterNetEvent('cdn-fuel:station:server:Withdraw', function(amount, location)
        ...
        local station = GetStation(location)
        if not amount or amount <= 0 or amount % 1 ~= 0 or not station then return end
        if tostring(station.owner) ~= tostring(Player.PlayerData.citizenid) then return end

        local StationBalance = tonumber(station.balance) or 0
    ...
    RegisterNetEvent('cdn-fuel:station:server:Deposit', function(amount, location)
        ...
        local setamount = ((tonumber(station.balance) or 0) + amount)
