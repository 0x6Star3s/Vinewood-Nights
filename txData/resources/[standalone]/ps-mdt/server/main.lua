...
RegisterNetEvent('mdt:server:removeMoney', function(citizenId, fine, incidentId)
	...
	-- mandat moze wystawic tylko sluzba (bylo: kazdy kazdemu)
	local Officer = QBCore.Functions.GetPlayer(src)
	if not Officer or not Player then return end
	if not Config.PoliceJobs[Officer.PlayerData.job.name] then return end

	fine = tonumber(fine)
	if not fine or fine <= 0 or fine % 1 ~= 0 or fine > 100000 then return end
