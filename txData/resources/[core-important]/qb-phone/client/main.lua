...
local function DisableDisplayControlActions()
    ...
    -- z klawiatura przekazywana do gry blokujemy strzal/celowanie/kolo broni/oslone/przeladowanie, zeby pisanie nie strzelalo
    for _, c in ipairs({24, 25, 37, 44, 45, 47, 58, 68, 69, 70, 92, 199}) do DisableControlAction(0, c, true) end
...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            SetNuiFocusKeepInput(true) -- klawiatura idzie tez do gry: mozna chodzic z telefonem (wylaczane w polu tekstowym)
            SendNUIMessage({
                action = "open",
...
-- ponytail: komenda przemianowana z 'phone' na 'usephone' i bez RegisterKeyMapping - stary zapisany bind M -> phone w profilu FiveM gracza przestaje dzialac; telefon tylko z itemu (slot 1-5)
...
RegisterNetEvent('qb-phone:client:UsePhone', function()
    ExecuteCommand('usephone')
end)

RegisterNUICallback('KeepInput', function(data, cb)
    SetNuiFocusKeepInput(data and data.keep == true)
    cb('ok')
end)
