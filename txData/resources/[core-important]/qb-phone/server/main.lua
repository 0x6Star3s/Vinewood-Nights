...
QBCore.Functions.CreateCallback('qb-phone:server:GetPhoneData', function(source, cb)
    ...
    if Player ~= nil then
        ...
        if invoices[1] ~= nil then
            -- ponytail: jedno zapytanie IN (...) zamiast SELECT-a per faktura
            local offlineSenders, placeholders = {}, {}
            for _, v in pairs(invoices) do
                local Ply = QBCore.Functions.GetPlayerByCitizenId(v.sender)
                if Ply ~= nil then
                    v.number = Ply.PlayerData.charinfo.phone
                elseif v.sender and not offlineSenders[v.sender] then
                    offlineSenders[v.sender] = true
                    placeholders[#placeholders+1] = v.sender
                end
            end

            local numbers = {}
            if #placeholders > 0 then
                local marks = string.rep('?', #placeholders, ',')
                local rows = MySQL.query.await('SELECT citizenid, charinfo FROM players WHERE citizenid IN (' .. marks .. ')', placeholders)
                for _, row in pairs(rows or {}) do
                    numbers[row.citizenid] = json.decode(row.charinfo).phone
                end
            end

            for _, v in pairs(invoices) do
                if v.number == nil then v.number = numbers[v.sender] end
            end
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if not Ply or not amount or amount <= 0 or amount % 1 ~= 0 then return cb(false, Invoices) end
    -- najpierw sciagamy pieniadze, dopiero potem prowizja i konto firmowe (bylo odwrotnie = kasa z niczego)
    if not Ply.Functions.RemoveMoney('bank', amount, "paid-invoice") then
        TriggerClientEvent('QBCore:Notify', source, 'Za malo pieniedzy na koncie', 'error')
        return cb(false, Invoices)
    end
...
QBCore.Functions.CreateCallback('qb-phone:server:GetContactPictures', function(_, cb, Chats)
    -- ponytail: jedno zapytanie na cala liste czatow zamiast jednego na czat
    local conditions, params = {}, {}
    for _, v in pairs(Chats) do
        if v.number then
            conditions[#conditions+1] = 'charinfo LIKE ?'
            params[#params+1] = '%' .. v.number .. '%'
        end
    end

    local pictures = {}
    if #conditions > 0 then
        local rows = MySQL.query.await('SELECT charinfo, metadata FROM players WHERE ' .. table.concat(conditions, ' OR '), params)
        for _, row in pairs(rows or {}) do
            local charinfo = json.decode(row.charinfo)
            local metadata = json.decode(row.metadata)
            if charinfo and charinfo.phone then
                pictures[tostring(charinfo.phone)] = metadata and metadata.phone and metadata.phone.profilepicture or "default"
            end
        end
    end

    for _, v in pairs(Chats) do
        v.picture = pictures[tostring(v.number)] or "default"
    end
...
QBCore.Functions.CreateCallback('qb-phone:server:FetchResult', function(_, cb, search)
    ...
    local searchData = {}
    local ApaData = {}
    -- Parametry zamiast sklejania stringa (escape_sqli nie escapowal backslasha)
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if sender.PlayerData.money.bank < amount then -- bylo bez sprawdzenia salda: bank schodzil na minus
        TriggerClientEvent('QBCore:Notify', src, "Not enough money on the bank account", "error")
        return
    end

    local query = '%"account":"' .. tostring(iban) .. '"%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if result[1] ~= nil then
        local reciever = QBCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
...
-- Telefon jako item do uzycia (slot 1 / klikniecie w ekwipunku); otwiera to samo co klawisz M
