...
local function Holster(shootbool)
    local ped = PlayerPedId()
...
-- ESC chowa trzymany item (bron) zamiast otwierac pause menu; bez itemu w rece ESC dziala normalnie
CreateThread(function()
    while true do
        if currentWeapon then
            DisableControlAction(0, 200, true)
            if IsDisabledControlJustPressed(0, 200) then Holster(false) end
            Wait(0)
        else
            Wait(250)
        end
