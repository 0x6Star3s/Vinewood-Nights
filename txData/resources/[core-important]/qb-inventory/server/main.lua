...
local function AddItem(source, item, amount, slot, info, created)
	...
	if amount <= 0 or amount % 1 ~= 0 then return false end -- ujemna/ulamkowa ilosc = duplikacja
...
---Wypelnia bagaznik zawartoscia z configu jobu. Tylko dla skryptow serwerowych - klient nie ma tu wstepu.
local function SetTrunkItems(plate, items)
	if type(plate) ~= 'string' or type(items) ~= 'table' then return false end
	Trunks[plate] = { items = items }
	return true
end

exports("SetTrunkItems", SetTrunkItems)
...
---Znajdz przepis po nazwie itemu (zrodlo prawdy = Config, nie klient)
local function GetRecipe(recipes, itemName)
	for _, recipe in pairs(recipes) do
		if recipe.name == itemName then return recipe end
	end
end

---Czy gracz ma wszystkie skladniki (RemoveItem zdejmuje z jednego slotu, wiec tak tez sprawdzamy)
local function HasCraftMaterials(src, costs, amount)
	for item, cost in pairs(costs) do
		local needed = cost * amount
		local enough = false
		for _, slotItem in pairs(GetItemsByName(src, item)) do
			if slotItem.amount >= needed then
				enough = true
				break
			end
		end
...
---@param repMeta string metadata z poziomem craftingu
---@param recipes table lista przepisow z Config
local function CraftFromConfig(src, recipes, repMeta, itemName, amount, toSlot)
	local Player = QBCore.Functions.GetPlayer(src)
	if not Player then return end

	amount = tonumber(amount)
	if not itemName or not amount or amount < 1 or amount % 1 ~= 0 then return end

	local recipe = GetRecipe(recipes, itemName)
	if not recipe then return end

	local rep = Player.PlayerData.metadata[repMeta] or 0
	if rep < (recipe.threshold or 0) then return end

	if not HasCraftMaterials(src, recipe.costs, amount) then
		QBCore.Functions.Notify(src, "You don't have the required materials", "error")
		return
	end

	for item, cost in pairs(recipe.costs) do
		if not RemoveItem(src, item, cost * amount) then return end
	end

	AddItem(src, itemName, amount, tonumber(toSlot))
	Player.Functions.SetMetaData(repMeta, rep + ((recipe.points or 0) * amount))
	TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
end

RegisterNetEvent('inventory:server:CraftItems', function(itemName, _, amount, toSlot)
	CraftFromConfig(source, Config.CraftingItems, "craftingrep", itemName, amount, toSlot)
end)

RegisterNetEvent('inventory:server:CraftAttachment', function(itemName, _, amount, toSlot)
	CraftFromConfig(source, Config.AttachmentCrafting["items"], "attachmentcraftingrep", itemName, amount, toSlot)
end)
...
---Zwraca asortyment sklepu z serwera (nil = brak zrodla, uzyjemy listy od klienta)
---@param id string np. "Itemshop_247supermarket", "Vendingshop_12", "Dealer_Jimmy", "Canteenshop_5"
function GetServerShopItems(id)
	local prefix = QBCore.Shared.SplitStr(tostring(id), "_")[1]
	local key = QBCore.Shared.SplitStr(tostring(id), "_")[2]

	if prefix == "Itemshop" and GetResourceState("qb-shops") == "started" then
		return exports["qb-shops"]:GetShopProducts(key)
	elseif prefix == "Dealer" and GetResourceState("qb-drugs") == "started" then
		return exports["qb-drugs"]:GetDealerProducts(key)
	elseif prefix == "Vendingshop" then
		return Config.VendingItem
	elseif (prefix == "Canteenshop" or prefix == "Slushyshop" or prefix == "Sodashop" or prefix == "Coffeeshop")
		and GetResourceState("qb-prison") == "started" then
		return exports["qb-prison"]:GetShopProducts(prefix)
	end
	-- police/hospital: asortyment zalezy od stopnia sluzbowego liczonego u klienta - do przerobienia osobno
	return nil
end
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			elseif name == "shop" then
				-- Asortyment i ceny bierzemy z Configu po stronie serwera; lista od klienta
				-- to tylko fallback dla sklepow, ktore nie maja jeszcze eksportu (audyt ekonomii pkt 2).
				local shopItems = GetServerShopItems(id) or other.items
				secondInv.name = "itemshop-"..id
				secondInv.label = other.label
				secondInv.maxweight = 900000
				secondInv.inventory = SetupShopItems(shopItems)
				ShopItems[id] = {}
				ShopItems[id].items = shopItems
				secondInv.slots = #shopItems
			...
			elseif name == "otherplayer" then
				...
				if OtherPlayer then
					local meta = OtherPlayer.PlayerData.metadata
					if not (meta["ishandcuffed"] or meta["isdead"] or meta["inlaststand"]) then return end
					local distance = #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(OtherPlayer.PlayerData.source)))
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		if itemInfo and (itemInfo["unique"] or itemInfo["type"] == "weapon") then
			fromAmount = 1
		end
		local bankBalance = Player.PlayerData.money["bank"]
		local price = tonumber(itemData.price) -- cena z ShopItems (serwer), nie z pakietu klienta
...
RegisterNetEvent('qb-inventory:server:SaveStashItems', function(stashId, items)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or type(stashId) ~= 'string' or type(items) ~= 'table' then return end

    -- zapis tylko do stasha, ktory gracz ma otwarty, albo do stasha swojego jobu (np. mechanicstash)
    local isOpenByPlayer = Stashes[stashId] and Stashes[stashId].isOpen == src
    if not isOpenByPlayer and stashId ~= Player.PlayerData.job.name .. 'stash' then return end

    local clean = {}
    for slot, item in pairs(items) do
        slot = tonumber(slot)
        local amount = type(item) == 'table' and tonumber(item.amount)
        if slot and amount and amount > 0 and amount % 1 == 0
            and type(item.name) == 'string' and QBCore.Shared.Items[item.name:lower()] then
            item.amount = amount
            item.slot = slot
            clean[slot] = item
        end
    ...
    Stashes[stashId].items = clean
    SaveStashItems(stashId, clean)
end)
	...
	if Player then
			if itemData then
				...
				elseif itemData["name"] == "syphoningkit" then
					info.gasamount = 0
				elseif itemData["name"] == "jerry_can" then
					info.gasamount = 0
				elseif itemData["name"] == "markedbills" then
					info.worth = math.random(5000, 10000)
				elseif itemData["name"] == "labkey" then
					if GetResourceState("qb-methlab") == "started" then
