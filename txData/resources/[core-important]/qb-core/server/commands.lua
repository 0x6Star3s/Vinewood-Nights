...
-- ==========================================================================================
-- EKONOMIA: mnoznik zarobkow w locie (audyt ekonomii 2026-09-02)
--   /boost 3       -> x3
--   /boost reset   -> powrot do QBConfig.Economy.DefaultBoost
--   /ekonomia      -> podglad ustawien
-- Mnoznik dziala od razu, bez restartu zasobow. Po restarcie serwera wraca wartosc z config.lua.
-- ==========================================================================================

QBCore.Commands.Add('boost', 'Mnoznik zarobkow (1.0-4.0 albo "reset")', {
    { name = 'mnoznik', help = 'np. 3 albo reset' }
}, true, function(source, args)
    local economy = QBCore.Config.Economy
    local input = tostring(args[1]):lower()
    local newBoost

    if input == 'reset' then
        newBoost = economy.DefaultBoost
    else
        newBoost = tonumber(args[1])
        if not newBoost then
            TriggerClientEvent('QBCore:Notify', source, 'Podaj liczbe (np. /boost 3) albo /boost reset', 'error')
            return
        end
    end

    economy.Boost = math.min(economy.MaxBoost, math.max(economy.MinBoost, newBoost))

    local who = source > 0 and GetPlayerName(source) or 'konsola'
    print(('[EKONOMIA] %s ustawil mnoznik zarobkow na x%.1f'):format(who, economy.Boost))
    TriggerEvent('qb-log:server:CreateLog', 'adminmenu', 'Ekonomia', 'yellow',
        ('%s ustawil mnoznik zarobkow na x%.1f'):format(who, economy.Boost), false)
    TriggerClientEvent('QBCore:Notify', -1,
        ('[EKONOMIA] Mnoznik zarobkow: x%.1f'):format(economy.Boost), 'success')
end, 'admin')

QBCore.Commands.Add('ekonomia', 'Pokaz ustawienia ekonomii', {}, false, function(source)
    local economy = QBCore.Config.Economy
    local heists = economy.Heists
    local text = ('Mnoznik zarobkow: x%.1f (domyslny x%.1f, zakres %.1f-%.1f) | Napady dev: %s | Fleeca $%d/%dmin, Paleto $%d/%dmin, Pacific $%d/%dmin'):format(
        economy.Boost, economy.DefaultBoost, economy.MinBoost, economy.MaxBoost,
        economy.DevHeists and 'TAK' or 'NIE',
        heists.fleeca.payout, heists.fleeca.cooldown,
        heists.paleto.payout, heists.paleto.cooldown,
        heists.pacific.payout, heists.pacific.cooldown)

    if source > 0 then
        TriggerClientEvent('QBCore:Notify', source, text, 'primary', 12000)
    end
    print('[EKONOMIA] ' .. text)
end, 'admin')
