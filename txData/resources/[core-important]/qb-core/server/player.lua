...
function QBCore.Player.Save(source)
    ...
    -- przy playerDropped ped juz nie istnieje: GetEntityCoords(0) wywalalo cala funkcje i zapis
    -- (w tym pieniadze z ostatnich minut) przepadal. Wtedy bierzemy ostatnia znana pozycje.
    local ped = GetPlayerPed(source)
    local ok, pcoords = pcall(GetEntityCoords, ped)
    if not ok or ped == 0 or not pcoords then
