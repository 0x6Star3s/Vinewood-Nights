...
---Przelicza wyplate przez mnoznik ekonomii (QBConfig.Economy.Boost, komenda /boost).
---Uzywac WYLACZNIE na dochodach liczonych po stronie serwera - nigdy na kwocie przyslanej przez klienta.
---@param amount number kwota bazowa
---@return number kwota po mnozniku
function QBCore.Functions.ScalePayout(amount)
    local base = tonumber(amount) or 0
    if base <= 0 then return 0 end
    local boost = QBCore.Config.Economy and QBCore.Config.Economy.Boost or 1.0
    return math.floor(base * boost)
end

exports('ScalePayout', QBCore.Functions.ScalePayout)
    ...
    if next(QBCore.Players) then
        for _, Player in pairs(QBCore.Players) do
            if Player then
                ...
                payment = QBCore.Functions.ScalePayout(payment) -- mnoznik ekonomii (/boost)
...
function QBCore.Functions.Kick(source, reason, setKickReason, deferrals)
    ...
    CreateThread(function()
        ...
        for _ = 0, 4 do
            Wait(5000)
            if GetPlayerPing(source) <= 0 then break end -- gracz juz wyszedl
            DropPlayer(source, reason)
        end
    end)
end
