...
-- Pieniadze na koncie zapisujemy od razu, nie czekajac na okresowy zapis ani na wyjscie z gry -
-- wplata tuz przed rozlaczeniem przepadala, bo zapis przy playerDropped potrafil sie nie wykonac.
AddEventHandler('QBCore:Server:OnMoneyChange', function(src, moneytype)
    if moneytype ~= 'bank' then return end
    local Player = QBCore.Players[src]
    if Player then Player.Functions.Save() end
end)
